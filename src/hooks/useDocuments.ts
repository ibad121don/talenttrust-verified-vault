...
export const useDocuments = () => {
  ...
  const requestVerification = async (
    documentId: string,
    requestType: VerificationRequest["request_type"] = "ai_analysis"
  ) => {
    try {
      await documentService.requestVerification(
        documentId,
        requestType,
        currentUser
      );
      ...
      toast({
        ...
        description:
          "Your document has been sent to TalentTrust AI for verification. You'll be notified when complete.",
      });
    } catch (error) {
