...
import { FileText, Share, Copy, Eye, CheckCircle, Shield, Globe, Lock } from "lucide-react";
...
const SharePortfolioDialog = ({ isOpen, onClose, documents }: SharePortfolioDialogProps) => {
  ...
  const generateShareUrl = async () => {
    ...
    try {
      // Simulate URL generation (in real app, this would call an API)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a shareable link to showcase your verified credentials to employers or institutions.
          </DialogDescription>
        </DialogHeader>
                      ...
                      filteredDocuments.map((doc, index) => (
                        <div key={doc.id} className="flex items-center space-x-3 p-2 bg-gray-50 rounded">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-600 truncate">{doc.issuer}</p>
                          </div>
                          <div className="flex items-center space-x-1">
                            <Badge className={`${getDocumentTypeColor(doc.type)} text-xs`}>
                              {doc.type.replace('_', ' ')}
                            </Badge>
                            {doc.status === 'verified' && (
